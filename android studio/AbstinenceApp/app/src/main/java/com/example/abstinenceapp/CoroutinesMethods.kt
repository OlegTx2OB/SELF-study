...
class CoroutinesMethods
{
    companion object
    {
        ...
        fun setClockTimeThread(context: Context, isLoopActive: Boolean,
            ...
            runnable = kotlinx.coroutines.Runnable {
                GlobalScope.launch(Dispatchers.Main) {
                    while (isLoopActive) {
                        ...
                        val sPTimeInMin = sP.getInt("sPTimeInMin", currTimeInMin)
                        if (sPTimeInMin == currTimeInMin) TimeMethods.putIntToSP(context, "sPTimeInMin", currTimeInMin)

                        mMainClockTV.text = TimeMethods.setTimeToMainClock(currTimeInMin, sPTimeInMin)
                        activeClockRing.progress = (currTimeInMin - sPTimeInMin) % 1440
                        delay(2000)
                    }
                }
            }
